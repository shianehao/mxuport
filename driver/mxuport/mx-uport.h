...
#define TRIGGER_SEND_NEXT           512 //4096
...
/*
 * USB vendor request for UPort G2 only
 */
#define RQ_VENDOR_GET_SYS_REG               0x34

/*
 * Definitions for GET_SYS_REG
 */
#define PWR_REMAP_BIT                       0x3
#define REMAP_TO_ROM                        0x0
#define REMAP_TO_XIP                        0x1
#define REMAP_TO_SRAM                       0x2
...
struct mxuport_serial
{
	...
	int					isUsbUart; /* is MUX52U UART */
	int					isFWVersionMatch;
	struct work_struct			work_fwupg; /* work_struct for fw upgrade */
	struct delayed_work			dw_usbreset; /* work_struct for checking usb detect by host after fw upgrade. In some platform host may not detect usb device reset. */
};
...
static void mxuport_set_termios (struct tty_struct *tty, struct usb_serial_port *port, struct ktermios *old);
#else
static void mxuport_set_termios (struct tty_struct *tty, struct usb_serial_port *port, const struct ktermios *old);
#endif
